...
faultOptions = { ...
    'A-G (Phase A to Ground)', ...             % LG
    'B-G (Phase B to Ground)', ...
    'C-G (Phase C to Ground)', ...
    'A-B (Phase A to B)', ...                  % LL
    'B-C (Phase B to C)', ...
    'C-A (Phase C to A)', ...
    'A-B-G (Phase A–B to Ground)', ...         % LLG
    'B-C-G (Phase B–C to Ground)', ...
    'C-A-G (Phase C–A to Ground)', ...
    'A-B-C (Three-phase)', ...                 % LLL
    'A-B-C-G (Three-phase to Ground)', ...     % LLLG
    'None (No fault)'};
[idx, tf] = listdlg(...
...
% Default fault parameters (modifiable)
fault_start_time = 0.2;    % seconds
fault_duration   = 0.1;    % seconds
fault_magnitude  = 8.0;    % per unit
...
%% 1-Cycle DFT Phasor Estimation for Each Phase
mag   = zeros(3, N);
phase = zeros(3, N);
realC = zeros(3, N);
imagC = zeros(3, N);
...
%% Apply Protection Logic per Phase	rip = false(3, N);
for ph = 1:3
